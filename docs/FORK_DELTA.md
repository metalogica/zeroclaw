...
| FD-07 | Laminar observability layer re-home onto the v0.8.0 pipeline | zc-jfun, zc-gnpx, zc-a1bp | zeroclaw-api, zeroclaw-runtime, zeroclaw-gateway, zeroclaw-channels | private | v0.8.0 deleted the span-*producing* half of observability; this re-homes it. Re-adds `Observer::start_activation` + `NoopSpan` (zeroclaw-api), ports `OtelSpan` (root/child, `lmnr.association.properties.session_id` twin, per-span `deployment.environment`) onto the zc-ju48 shared-provider `OnceLock` (otel.rs), and mints `agent.activation` roots + `llm.call`/`tool.call` mirrors + `stamp_turn_exit` at EVERY prod ingress owner — `execute_turn` (ACP/rpc), gateway `run_gateway_chat_with_tools` (POST /api/chat), channels orchestrator + 42618 webhook (shared `run_tool_call_loop`), CLI `run` — across BOTH `agent.rs turn/turn_streamed` engines and `loop_.rs run_tool_call_loop`, so prod-relay traces are not hollow (§7 must-not-break / FMEA #4). Guards ported verbatim: `session_id` absence-not-empty (None on /api/chat, never synthesized), `user_id` only past the `CLAW_USER_ID` 32-char gate; scrub+truncate once/site; tool-call-only `name(args)` summaries. `deployment.environment` sourced from `ZEROCLAW_/OTEL_DEPLOYMENT_ENVIRONMENT` env (v0.8.0 config lacks the field — see zc-t0ii follow-up). `.with_headers()` left as-is (already upstream). **FOLLOW-UP zc-gnpx (root I/O):** the initial squash wired `lmnr.span.input/output` only on `llm.call` child spans, leaving the `agent.activation` ROOT's input/output empty — a fail against §1.3/§4.4 "root input/output non-empty", caught by the zc-b78l live battery. Added `stamp_root_input`/`stamp_root_output` ambient helpers (`current_span()`; scrub+truncate once/site, PII-safe) called at loop entry + every final-answer terminal across all three engines (`turn`, `turn_streamed`, `run_tool_call_loop`), so Laminar's root-span input/output columns populate. Second commit under this row. **FOLLOW-UP zc-a1bp (ws root):** the original owner-list omitted `/ws/chat` — `ws::handle_ws_chat` called `turn_streamed` with no `start_activation`/`scope_span`, so the ws surface (a primary clawcraft streaming ingress) emitted only upstream-native `gen_ai.*` spans, no `agent.activation`/`llm.call`, and the zc-gnpx root mirrors were inert there (caught by the zc-b78l re-run). Ported the root mint into `ws::handle_ws_chat` (`start_activation(WebChat, session_key)` + `tag_channel("web")`/`tag_user_id` + `scope_span` the streamed turn), completing the "every prod ingress owner" claim. Third commit under this row (FD-07 carries 3 commits; multi-commit-per-row is allowed, as FD-04). | — | — |
...
| FD-10 | conflict-canary.yml — weekly informational merge-tree walk | zc-p7tx | .github/workflows | private | Weekly (`schedule` cron + `workflow_dispatch`, `fetch-depth: 0`) canary that replays each cumulative sovereign tip against the latest upstream main with `git merge-tree --write-tree`, attributes every conflicted file to its `Fork-Delta:` FD id, writes `GITHUB_STEP_SUMMARY` (upstream SHA · per-FD file list · total), and maintains a SINGLE pinned tracking issue while conflicts > 0. Informational only — the analysis job always exits 0 on a successful walk (a conflict is data, never a red gate; FMEA #9); the only non-zero exit is a genuine git/gh tooling error. Diffs against `refs/remotes/upstream/main` (fetched fresh), never bare `upstream`. | — | — |
