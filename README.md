...
#### Change Log
* **0.6.6-alpha-p3**: Use Wolfi insteads of distroless container image.
* **0.6.6-alpha-p2**: Fix a bug in the non_cli_excluded_tools. The `/api/tools` and are callable despite being in the exclusion list. REMOVED (fixed in upstream)
* **0.6.6-alpha-p1**: Adds a pre-shared token functionality. This allows the claw instance to be pre-seeded with a pairing token to allow faster cold starts and simpler infrastructure design through configmap injection of tokens for cloud-provisioned services.
