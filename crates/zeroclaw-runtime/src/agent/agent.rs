...
impl Agent {
    ...
    // Port of the run_tool_call_loop integration onto the turn/turn_streamed
    // loop so the WS gateway and ACP surfaces enforce praxis continuations
    // too. The per-turn FSM lives in `continuation.rs` and is shared; only the
    // wiring differs — ConversationMessage history, `execute_tool_call`
    // (hook/span parity with model-initiated calls), and `TurnEvent`
    // forwarding so WS clients see runtime-driven calls.
                ...
                let outcome = self
                    .execute_tool_call(
                        &ParsedToolCall {
                            ...
                            tool_call_id: Some(Self::new_turn_id()),
                        },
                        turn_id,
                    )
                    .await;
    ...
    async fn enforce_continuation_on_turn_end(
        ...
        self.drive_continuation_calls(continuation, event_tx, turn_id, trace_channel)
            .await?;
    ...
    pub async fn turn(&mut self, user_message: &str) -> Result<String> {
        ...
        for iteration in 0..self.config.resolved.max_tool_iterations {
            ...
            // never while a continuation is pending — a cached reply would bypass the termination
            // guard). The key must include the whole provider-visible transcript, not just the
            // last user message, otherwise distinct conversations can collide when their final
            // prompt matches.
            let cache_key = if continuation.has_pending() {
                None
            } else {
            ...
            if calls.is_empty() {
                ...
                // Turn-termination guard (rnk-h6g3): the model wants to stop
                // while a praxis continuation is pending. Refuse, drive/force,
                // or route the safety net before allowing the turn to end.
                match self
            ...
            self.drive_continuation_calls(&mut continuation, None, &turn_id, "agent_turn")
                .await?;
    ...
    pub async fn turn_streamed_with_steering_state(
        ...
        for iteration in 0..self.config.resolved.max_tool_iterations {
            ...
            {
                // Cancel precedence (decision §3.2): cancel wins over the
                // termination guard, but a pending continuation is still never
                // dropped silently — fire the safety net on the pending bead,
                // THEN yield the cancellation.
                if continuation.has_pending() {
                    self.route_continuation_safety_net(
                        &mut continuation,
            ...
            // include the full provider-visible transcript.
            let cache_key = if continuation.has_pending() {
                None
            } else {
                self.response_cache_key_for_messages(&prepared_messages, &effective_model)
            ...
            if calls.is_empty() {
                ...
                match self
                    ...
                    .map_err(|error| StreamedTurnError {
                        error,
                        committed_response: committed_response.clone(),
...
mod tests {
    ...
    // ── NextAction continuation auto-drive (rnk-h6g3 / zc-g50j) ───────────
    // Port of the run_tool_call_loop scenario tests onto the
    ...
    // outputs staying unaffected. Plus a cancel-precedence test (safety net
    // fires then the turn yields) and a poison-`data` test (an undeserializable
    // `data` field on a null envelope still ends the turn cleanly — the guard
    // never inspects `data` on a null next_action).
    mod continuation_auto_drive {
        ...
        /// left at the trait default (empty stream, `supports_streaming` =
        /// false), so `turn_streamed` exercises its non-streaming fallback
        /// through the same script.
        struct ContinuationProvider {
            responses: Mutex<VecDeque<zeroclaw_providers::ChatResponse>>,
            requests: Arc<Mutex<Vec<Vec<ChatMessage>>>>,
            /// Optionally cancel this token on the Nth (0-based) `chat` call,
            /// checked *before* returning the scripted response. Used by the
            /// cancel-precedence scenario.
            cancel_on_call: Option<(usize, tokio_util::sync::CancellationToken)>,
            call_count: AtomicUsize,
        }

        impl ContinuationProvider {
            fn new(responses: Vec<zeroclaw_providers::ChatResponse>) -> Self {
                Self {
        ...
        fn shell_call(id: &str, command: &str) -> zeroclaw_providers::ChatResponse {
            zeroclaw_providers::ChatResponse {
                text: Some(String::new()),
                tool_calls: vec![zeroclaw_providers::ToolCall {
                    id: id.to_string(),
        ...
        fn text(content: &str) -> zeroclaw_providers::ChatResponse {
            zeroclaw_providers::ChatResponse {
        ...
        async fn run_turn(agent: &mut Agent, streamed: bool) -> Result<String> {
            if streamed {
                ...
                agent
                    .turn_streamed("walk the execution", tx, None)
                    .await
        ...
        async fn cancel_fires_safety_net_then_yields() {
            // Cancel precedence (decision §3.2): a cancel arriving while an
            // agent_work continuation is pending must fire the safety net on
            // the pending bead, THEN yield the cancellation — cancel wins over
            // the termination guard. The provider cancels the token on its
            // second `chat` call, i.e. after iteration 0 established a pending
            // agent_work continuation and the loop returns to the top-of-loop
            // cancel check.
            let cancel_token = tokio_util::sync::CancellationToken::new();
            let (shell, commands) = ScriptedShellTool::new(vec![
                ("--state waiting_for", envelope(serde_json::Value::Null)),
            ...
            let provider = ContinuationProvider::with_cancel_on_call(
                vec![
                    ...
                    // the next iteration fires the safety net.
                    text("stalling"),
                ],
                1,
                cancel_token.clone(),
            ...
            assert!(
                crate::agent::loop_::is_tool_loop_cancelled(&err.error),
                "cancel must yield the ToolLoopCancelled error: {}",
                err.error
            );
            let commands = commands.lock();
            assert!(
        ...
        #[tokio::test]
        async fn poison_data_on_null_envelope_ends_turn_cleanly() {
            // A null next_action ends the continuation regardless of `data`
            // being undeserializable garbage — the guard never inspects `data`
            // on a null next_action, so a poison `data` payload cannot wedge
            // the turn. update → (call execute) → null(with poison data).
            let poison_terminal =
                r#"{"data": {"nested": [1, {"weird": null}, " "], "cyclic": "\ud800"}, "next_action": null}"#
                    .to_string();
            let (shell, commands) = ScriptedShellTool::new(vec![
                (
                    "praxis update zc-e1",
                ...
                ("praxis execute", poison_terminal),
            ]);
            let provider = ContinuationProvider::new(vec![
                shell_call(
                    "tc1",
                    "praxis update zc-e1 --state done --output '{}' --json",
                ),
                text("walk complete"),
            ]);
            let requests = Arc::clone(&provider.requests);
            ...
            let result = run_turn(&mut agent, false)
                .await
                .expect("turn should succeed even with a poison data payload");

            assert_eq!(result, "walk complete");
            assert_eq!(
                request_count(&requests),
                2,
                "the poison-data null envelope terminates the chain cleanly"
            );
            let commands = commands.lock();
            // No safety-net routing: the chain terminated normally.
            assert!(
                !commands.iter().any(|c| c.contains("--state waiting_for")),
                "a clean null-terminal must not trigger the safety net: {commands:?}"
            );
        }
    }
}
