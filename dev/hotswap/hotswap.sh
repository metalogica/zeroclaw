...
# Self-verify marker: grep the freshly built binary for this string to prove the
# change is in the build. Defaults to the Laminar span marker (post-v0.8.0);
# override for ad-hoc probes: HOTSWAP_MARKER="my-probe" dev/hotswap/hotswap.sh
HOTSWAP_MARKER="${HOTSWAP_MARKER:-lmnr.span.input}"
...
# 0b. One-time migration reset (RESET_VOLUMES=1). After the v0.8.0 multi-crate
#     move, the old single-crate hotswap caches (registry/git/target) and the
#     pinned hotswap-base image are stale — a fresh incremental build off them
#     can wedge. Clearing them forces a clean re-pin from the new :dev image.
if [ "${RESET_VOLUMES:-0}" = "1" ]; then
    bold "▶ RESET_VOLUMES=1 — clearing hotswap caches + base image for a clean re-pin…"
    docker volume rm zeroclaw-hotswap-registry zeroclaw-hotswap-git zeroclaw-hotswap-target 2>/dev/null || true
    docker rmi clawcraft-claw-runtime:hotswap-base 2>/dev/null || true
fi
...
time docker run --rm \
    ...
    -w /app \
    "$BUILDER_IMG" \
    cargo build --locked -p zeroclawlabs --bin zeroclaw --features "$FEATURES"
...
probe_hits="$(grep -ac "$HOTSWAP_MARKER" "$BIN_OUT" || true)"
if [ "${probe_hits:-0}" -gt 0 ]; then
    bold "✓ built binary contains '$HOTSWAP_MARKER' ($probe_hits hits) — your change is in this build"
else
    echo "ℹ built binary has no '$HOTSWAP_MARKER' marker (expected once the temporary probes are removed)"
fi
